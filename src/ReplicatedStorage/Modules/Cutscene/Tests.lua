...
return function()
	...
	local status = runner:Play(scene({
		Steps.Call(function(context)
			context:Defer(function() table.insert(order, 'cleanup1') end)
			context:Defer(function() table.insert(order, 'cleanup2') end)
			table.insert(order, 'step1')
		end),
		Steps.Camera(CFrame.new(0, 10, 0), 0, 45),
		Steps.Wait(0.02),
		Steps.Call(function() table.insert(order, 'step2') end),
	}))
	assert(status == 'Completed', 'Sequence did not complete')
	assert(table.concat(order, ',') == 'step1,step2,cleanup2,cleanup1', 'Steps or cleanup ran out of order')
	restored(runner)

	local secondRunner = Cutscene.new()
	status = runner:Play(scene({
		Steps.Call(function()
			assert(secondRunner:Play(scene({})) == 'Busy', 'Concurrent camera ownership was allowed')
			runner:Cancel('Test cancellation')
		end),
		Steps.Call(function() error('Must not run after cancellation') end),
	}))
	...
	local failed, message = runner:Play(scene({
		...
		Steps.Call(function() error('Expected failure') end),
	}))
	assert(failed == 'Failed' and string.find(message, 'Expected failure', 1, true), 'Step failure was not reported')
	restored(runner)

	local callback, stopped = nil, 0
	status = runner:Play(scene({ Steps.Call(function(context)
	...
	status = runner:Play(scene({ Steps.Call(function(context)
		context:Defer(function() cleanupRan = true end)
		context:Defer(function() error('Expected cleanup failure') end)
	end) }))
	assert(status == 'Failed' and cleanupRan, 'A failing cleanup must not prevent later cleanup')
	restored(runner)

	local setupRestored = false
	local setupFailure = Cutscene.new({ Setup = function(context)
		context:Defer(function() setupRestored = true end)
	...
	status = runner:Play(scene({
		Steps.Call(function()
			task.delay(0.02, function() runner:Cancel('Cancel tween') end)
		end),
		Steps.Camera(CFrame.new(0, 50, 0), 2, 40),
		Steps.Call(function() tweenFinished = true end),
	}))
	assert(status == 'Cancelled' and not tweenFinished, 'Tween cancellation ran later steps')
	restored(runner)

	status = runner:Play(scene({ Steps.Call(function(context)
		task.delay(0.02, function() runner:Destroy() end)
		context:Wait(2)
	end) }))
