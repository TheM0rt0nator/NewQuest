...
function CinematicBars.new(options)
	...
	gui.Parent = assert(Players.LocalPlayer:FindFirstChildOfClass('PlayerGui'), 'PlayerGui unavailable')
	return self
end
...
function CinematicBars:_Animate(top, bottom)
	...
	for bar, position in { [self.Top] = top, [self.Bottom] = bottom } do
		local tween = TweenService:Create(bar,
			TweenInfo.new(self.config.animationTime, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
