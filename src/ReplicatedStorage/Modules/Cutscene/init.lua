...
function Context:Wait(seconds)
	assert(type(seconds) == 'number' and seconds >= 0 and seconds < math.huge, 'Invalid wait duration')
...
function Context:Await(start, timeout)
	...
	self:Check()
	release()
	if failure then error(tostring(failure), 0) end
end
...
function Context:GetCFrame(target)
	...
	assert(typeof(target) == 'Instance', 'Expected a CFrame, marker name, Attachment, BasePart, or Model')
	if target:IsA('Attachment') then return target.WorldCFrame end
...
local function setup(context, options)
	...
	context:Defer(function() camera.CameraType = cameraType end)
	...
	context:Defer(function() camera.CFrame = cframe end)
	context:Defer(function() camera.Focus = focus end)
	context:Defer(function() camera.FieldOfView = fov end)
	...
	for _, signal in { context.Player.CharacterRemoving, context.Player.CharacterAdded } do
		...
		context:Defer(function() connection:Disconnect() end)
	end
	...
	if humanoid then
		...
		context:Defer(function() connection:Disconnect() end)
		if humanoid.Health <= 0 then cancelForCharacter() end
	end
	...
	context:Defer(function() cameraChanged:Disconnect() end)

	if options.Setup then options.Setup(context) end
	context:Check()
	if context.Scene.Bars ~= false then
		...
		context:Defer(function() context.Bars:Destroy() end)
		context.Bars:Show()
...
function Cutscene:Play(scene, data)
	if self.Destroyed then return 'Failed', 'Cutscene runner was destroyed' end
	if activeContext then return 'Busy', 'Another cutscene is playing' end
	...
	local ok, failure = xpcall(function()
		...
		if scene.Validate then scene.Validate(context) end
		context:Check()
	...
	for index = #context.Cleanups, 1, -1 do
		...
		if not cleaned then table.insert(cleanupErrors, tostring(message)) end
	end
	self.Context, activeContext = nil, nil
	if #cleanupErrors > 0 then
		return 'Failed', (not ok and tostring(failure) .. '\n' or '') .. table.concat(cleanupErrors, '\n')
	end
	if context.Cancelled then return 'Cancelled', context.Reason end
	if not ok then return 'Failed', failure end
	return 'Completed'
end
