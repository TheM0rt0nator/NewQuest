...
local function playIntro()
	...
	if
		...
		and (
			...
			or (startingState == 4 or startingState == 5)
				and policeIntroFinishedCharacter == player.Character
	...
	local ok, status, reason = xpcall(function()
		...
		local function coverDream(check)
			if not dream then
				dream = DreamTransition.new(playerGui)
		...
		-- Leo's scene is parked until we add the police roleplay chapter.
		...
		if
			not policeRequested and not finaleRequested
		...
		if finaleRequested then
			return CutsceneManager.Play(ClassroomFinale(stage, function(context)
				if dream then
					dream:Reveal(function()
						context:Check()
					end)
			...
			end), { StudentSeat = seat })
		end
		if policeRequested then
			return CutsceneManager.Play(PoliceClassroom(stage, function(context)
				dream:Reveal(function()
					context:Check()
				end)
				dream = nil
	...
	retry.Visible = (
		...
		or player:GetAttribute("QuestState") == 4 or player:GetAttribute("QuestState") == 5
