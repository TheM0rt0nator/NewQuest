...
local function releaseSeat(player, outcome)
	...
	-- The return scene ends at the student's desk, not at the classroom entrance.
	if
		session.questState == 5
		and (outcome == "Completed" or outcome == "Skipped")
		and player.Character == session.character
		and session.humanoid.Health > 0
		and session.humanoid.SeatPart == session.seat
		and QuestService:CompleteObjective(player, "AnniversaryQuest", session.questState)
	then
		...
		session.character:SetAttribute("QuestResumeState", 6)
		-- Retain the seat and sitting animation until the player gets up or respawns.
		session.leftSeat = session.humanoid.Seated:Connect(function(active)
			if not active then
				releaseSeat(player)
			end
		end)
...
beginSeating.OnServerInvoke = function(player)
	...
	if
		retained and retained.finished and not retained.releasing
...
ReplicatedStorage:WaitForChild("ReturnToBerryAvenue").OnServerEvent:Connect(QuestReturnService.Return)
